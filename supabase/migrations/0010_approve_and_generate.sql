-- ============================================================================
-- OREoS 0010_approve_and_generate — the HITL gate between idea approval and
-- asset generation. Clients cannot touch pgmq or generation_jobs directly;
-- this SECURITY DEFINER RPC is the single entry point:
--   editor check → credit gate → ideas proposed→approved → jobs + pgmq.send.
-- Idempotent per (campaign_id, idea_id, format) via the jobs unique key.
-- ============================================================================
...
revoke all on function public.approve_and_generate(uuid[]) from public;
grant execute on function public.approve_and_generate(uuid[]) to authenticated;

-- Reject is a plain client-side update (ideas_guard allows proposed→rejected),
-- no RPC needed.
