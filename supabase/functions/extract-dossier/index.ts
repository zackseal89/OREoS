...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parts: Part[] = [];
    // Real product images, extracted from the source (not generated by Gemini).
    // Persisted onto the dossier as `assetUrls` for the review panel.
    ...
    if (product.source_type === "url" && product.source_url) {
      // Firecrawl gives us clean markdown + structured product data, which is
      // far more reliable than Gemini's own url_context fetch on real
      // e-commerce pages. We feed the scraped content in as text.
      const scraped = await scrapeUrl(product.source_url);
      assetUrls = scraped.images;
      const sections = [
        `Source URL: ${product.source_url}`,
        scraped.title ? `Page title: ${scraped.title}` : null,
        scraped.product
          ? `Structured product data:\n${JSON.stringify(scraped.product, null, 2)}`
          : null,
    ...
    // Attach the extracted product images (deterministic scrape data) alongside
    // the model's brand analysis. The review panel renders these as assetUrls.
    const dossierWithAssets = { ...dossier, assetUrls };

    // Pipeline-owned write: service role bypasses the products_guard lock on
    // status/dossier.
    const { error: writeError } = await svc
      .from("products")
      .update({ dossier: dossierWithAssets, status: "ready" })
      .eq("id", productId);
    if (writeError) throw writeError;

    return json({ ok: true, dossier: dossierWithAssets });
  } catch (err) {
